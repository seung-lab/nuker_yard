...
def rescale(img, old_range, new_range=[-1, 1]):
    ...
    if old_range[1] == old_range[0]:
        raise ('Invalid input value range: ', old_range)
    img *= (new_range[1] - new_range[0]) / (old_range[1] - old_range[0])
    img += new_range[0]
    return img

def get_voxels_for_stats(img, min_max_invalid = [True,True], percentile_ignore = [0,100], debug = False):
    r'''
    percentile_ignore is applied after applying min_max_invalid, in other words,
    the percentiles are computed from an image with the invalid min/max already masked out.
    '''
    ...
    low_ignore, high_ignore = percentile_ignore
    if (low_ignore > 0 or high_ignore <100) and stat_img.size > 0:
        low, high = np.quantile(stat_img, [0 if low_ignore is None else low_ignore/100.,  0 if high_ignore is None else high_ignore/100.])
        if low_ignore > 0:
            mask = np.logical_and(mask, img >= low)
        if high_ignore <100:
            mask = np.logical_and(mask, img <= high)
...
def normalize(img, method, target_scale = [-1,1], min_max_invalid = [True,True], invalid_values = [],
              percentile_ignore = [0,100], clip_percentile = [None,None], do_clipping = False, make_copy=True, debug=False):
    r'''
    *Assuming floating point voxel values.*
    '''
    stat_img = get_voxels_for_stats(img, min_max_invalid = min_max_invalid, percentile_ignore = percentile_ignore, debug=debug)
...
def normalize_section_shang(image: np.ndarray, nominalmin: float,
    ...
    arr = normalize(arr, normalization, target_scale = [nominalmin, nominalmax], min_max_invalid = [True]*2,
        do_clipping=clipvalues, make_copy=False)
...
def test_linear_equivalency(x, y):
    UNFINISHED
    r'''
    Whether x and y are linear remaps of each other in terms of element values at same locations.
    '''
    mi1 = np.min(x)
    ma1 = np.max(x)
    mi1 = np.min(x)
...
def test2_normalize_fill():
    UNFINISHED
    from numpy.random import randint
    x = np.random.rand(*randint(200, size=randint(5)))
    print(x.shape)
    y = normalize(x, 'fill', target_scale = [-5,5], min_max_invalid = [True,True])
